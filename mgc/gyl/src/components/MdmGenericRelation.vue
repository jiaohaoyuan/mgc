...
const fetchList = async () => {
  loading.value = true
  // Handle specific relation keywords if they are not just 'keyword'
  const params = { ...queryParams }
  if (params.keyword && props.entityName.includes('仓库')) {
    params.warehouseKeyword = params.keyword
    params.skuKeyword = params.keyword
  } else if (params.keyword && props.entityName.includes('组织')) {
    params.orgKeyword = params.keyword
    params.resellerKeyword = params.keyword
  } else if (params.keyword && props.entityName.includes('产品')) {
    params.productKeyword = params.keyword
    params.skuKeyword = params.keyword
  }
  
  try {
    const res = await axios.get(props.apiPath, { params })
  ...
  <div class="mdm-container">
    <div class="page-header">
      <div class="page-title">
        ...
        <div>
          <h2>{{ entityName }}管理</h2>
          <p class="subtitle">Master Data · {{ entityName }} Relation</p>
        </div>
      </div>
      <div class="header-actions">
        ...
        <el-button type="primary" :icon="Plus" @click="openAdd">新增关系</el-button>
      </div>
    </div>
    ...
    <div class="filter-bar">
      <el-form :inline="true" :model="queryParams" @submit.prevent="handleSearch" class="filter-form">
        ...
        <el-form-item label="效期状态">
          <el-select v-model="queryParams.dateStatus" placeholder="全部" clearable style="width: 110px" @change="handleSearch">
    ...
    <el-table v-loading="loading" :data="tableData" border stripe @selection-change="handleSelectionChange" class="sku-table">
      ...
      <el-table-column label="生效开始日期" width="120" align="center">
        <template #default="{ row }">{{ row.begin_date ? row.begin_date.split('T')[0] : '-' }}</template>
      </el-table-column>
      <el-table-column label="生效结束日期" width="120" align="center">
        <template #default="{ row }">{{ row.end_date ? row.end_date.split('T')[0] : '-' }}</template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="(isEdit ? '编辑' : '新增') + entityName + '关系'" width="560px" draggable destroy-on-close class="sku-dialog">
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="100px">
        <slot name="form" :form="form" :isEdit="isEdit"></slot>
        <el-row :gutter="12">
          <el-col :span="12">
            <el-form-item label="生效开始" prop="begin_date">
              <el-date-picker v-model="form.begin_date" type="date" value-format="YYYY-MM-DD" placeholder="开始日期" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="生效结束" prop="end_date">
...
.sku-dialog :deep(.el-dialog__header) { background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); border-radius: 12px 12px 0 0; }
