...
const fetchRows = async () => {
  loading.value = true
  try {
    const res = await axios.get('/export-tasks', { params: query })
...
const openDetail = async (id: number) => {
  const res = await axios.get(`/export-tasks/${id}`)
  detailData.value = res.data?.data || null
  detailVisible.value = true
}

const redownloadTaskFile = async (id: number) => {
  const res = await axios.get(`/export-tasks/${id}/download`)
  const payload = res.data?.data || {}
  const list = Array.isArray(payload.list) ? payload.list : []
  if (!list.length) {
    ElMessage.warning('任务无可下载数据')
    return
  }
  const xlsx = await import('xlsx')
  const ws = xlsx.utils.json_to_sheet(list)
  const wb = xlsx.utils.book_new()
  xlsx.utils.book_append_sheet(wb, ws, String(payload.bizType || '导出数据'))
  const now = new Date()
  const ts = `${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}${String(now.getSeconds()).padStart(2, '0')}`
  const baseName = String(payload.fileName || `export_task_${id}`).replace(/\.xlsx$/i, '')
  xlsx.writeFile(wb, `${baseName}_${ts}.xlsx`)
  ElMessage.success('已按任务配置重新导出')
}
  ...
  <div class="page-wrap">
    <el-card shadow="never">
      ...
      <el-table :data="rows" border v-loading="loading">
        ...
        <el-table-column label="导出数量" width="120">
        ...
        <el-table-column label="操作" width="190" fixed="right">
          <template #default="{ row }">
            ...
            <el-button link type="success" @click="redownloadTaskFile(row.id)" :disabled="row.status !== 'SUCCESS'">重新下载</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-drawer v-model="detailVisible" title="导出任务详情" size="50%">
      <template v-if="detailData">
        ...
        <el-divider content-position="left">筛选快照</el-divider>
        <pre class="json-box">{{ JSON.stringify(detailData.query_snapshot || {}, null, 2) }}</pre>
      </template>
    </el-drawer>
  </div>
</template>
